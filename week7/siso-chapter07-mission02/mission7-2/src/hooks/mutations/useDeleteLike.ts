...
function useDeleteLike() {
  return useMutation({
    ...
    //onMutate -> API 요청 이전에 호출되는 핀구
    //UI에 바로 변경을 보여주기 위해 cache업데이트
    onMutate: async (lp) => {
      //1. 이 게시글에 관련되 쿼리 취소(캐시된 데이터를 새로 불러오는 요청)
      ...
      //게시글 데이터를 복사해서 NewLpPoster라는 새로운 객체를 만들거다
      //복사하는 가장 큰 이유는 나중에 오류가 발생했을 때 이전상태로 되돌리기 위해서
      const newLpPost = { ...previousLpPost };

      //게시글에 저장된 좋아요 목록에서 현재 내가 눌렀던 위치를 찾아야함
      ...
      //업데이트된 게시글 데이터를 캐ㅣ에 저장
      //이렇게 하면 UI가 바로 업데이트 되고 사용자가 변화를 확인할 수 있다
