...
function useThrottle<T>(value: T, delay = 500): T {
  //1. 상태 변수 : throttledValue : 최종적을 쓰로틀링된 값
  //2. 초기값을 전달받은 value
  const [throttledValue, setThrottledValue] = useState<T>(value);

  //2. Ref lastExcuted : 마지막으로 실행된 시간을 기록하는 변수
  //useRef 사용하면 콤포넌트가 리렌더링 되어도 값이 유지되고, 변경되어도 리랜더링을 트리거 하지않음
  ...
  useEffect(() => {
    ...
    if (Date.now() >= lastExcuted.current + delay) {
      ...
      //최신 value를 throttledValue에 저장해서 컴포넌트에 리랜더링
      setThrottledValue(value);
      //setThrottledValue(() => value);
    } else {
      //충분한 시간이 지나지 않은경우, delay시간 후에 업데이트(최신 value로)
      const timerId = setTimeout(() => {
        //타이머가 만료되면, 마지막 업데이트 시간을 현재 시각으로 갱신한다.
        lastExcuted.current = Date.now();
        //최신 value를 throttledValue에 저장해서 컴포넌트에 리렌더링
        setThrottledValue(value);
        //setThrottledValue(() => value);
      }, delay);
